...
def parse_args():
    ...
    parser.add_argument("--grasp_offset", type=float, default=0.1)
    parser.add_argument("--pull_start", type=float, default=0.1)
    parser.add_argument("--pull_step", type=float, default=0.01)
    parser.add_argument("--pull_steps", type=int, default=30)
    parser.add_argument("--close_settle_steps", type=int, default=0)
...
def main():
    ...
    try:
        ...
        part["handle_out"] = float(args.direction_sign) * part["handle_out"]
        ...
        grasp_position = part["init_position"] + args.grasp_offset * part["handle_out"]
        for _ in range(10):
            step_num, _ = gym.control_to_pose(
        ...
        if args.close_settle_steps > 0:
            gym.run_steps(pre_steps=args.close_settle_steps, refresh_obs=False, print_step=False)
            step_num += args.close_settle_steps
        record_stage(rows, "gripper_closed", step_num, gym, part["init_position"])

        for i in range(args.pull_steps):
            step_num, _ = gym.control_to_pose(
        ...
        summary = {
            ...
            "pull_start": args.pull_start,
            "pull_step": args.pull_step,
            "pull_steps": args.pull_steps,
            "close_settle_steps": args.close_settle_steps,
