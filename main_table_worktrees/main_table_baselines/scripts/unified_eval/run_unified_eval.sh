...
RUN_DIR=${RUN_DIR:-$BASELINE_ROOT/results/unified_eval/run_$RUN_TAG}
...
log "RUN_DIR=$RUN_DIR"
log "ROWS=$ROWS ONLY_SAMPLE=${ONLY_SAMPLE:-<all>} EPISODES=$EPISODES DAMPS=$DAMPS MODES=$MODES GPU_ID=$GPU_ID"
log "EVALUATOR=$EVALUATOR"

manifest_args=(
  --rows "$ROWS"
  --data-manifest "$CANONICAL_MANIFEST"
  --output "$RUN_DIR/checkpoint_manifest.tsv"
)
if [[ -n "$ONLY_SAMPLE" ]]; then
  manifest_args+=(--only-sample "$ONLY_SAMPLE")
fi
python "$BASELINE_ROOT/scripts/unified_eval/make_checkpoint_manifest.py" "${manifest_args[@]}"   | tee "$RUN_DIR/meta/make_checkpoint_manifest.log"

cp "$CANONICAL_MANIFEST" "$RUN_DIR/meta/batch_manifest.csv"
...
python "$BASELINE_ROOT/scripts/unified_eval/summarize_unified_eval.py" "$RUN_DIR" | tee "$RUN_DIR/meta/summarize.log"
