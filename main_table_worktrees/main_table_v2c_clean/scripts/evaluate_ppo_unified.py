"""
Unified main-table evaluator for HandDrag PPO checkpoints.
...
def _load_running_mean_std(checkpoint, obs_dim):
    ...
    if rms is not None:
        return rms["running_mean"].float(), rms["running_var"].float()
    model = checkpoint.get("model", {})
    mean = model.get("running_mean_std.running_mean")
    var = model.get("running_mean_std.running_var")
...
class RLGamesActor(nn.Module):
    ...
    def __init__(self, checkpoint, device, stochastic=False):
        ...
        running_mean, running_var = _load_running_mean_std(checkpoint, obs_dim)
        # PICA no-GLA auxiliary checkpoints train with aux target channels
        # appended to the observation, but the flat policy MLP consumes only
        # the non-aux policy prefix. Keep eval policy normalization aligned
        # with actor_mlp.0 by slicing the RunningMeanStd prefix when needed.
        if running_mean.shape[0] > obs_dim:
            running_mean = running_mean[:obs_dim]
            running_var = running_var[:obs_dim]
    ...
    def forward(self, obs):
        if obs.shape[-1] != self.obs_dim:
            ...
            if 0 < pad_dim <= 8 and current_base_dim > 0:
                # Unified eval compatibility for older GLA/PICA checkpoints
                # whose base_obs had a few extra channels before the history
                # block. Insert zeros between base_obs and history/aux so
                # temporal tokens and aux targets remain aligned.
        ...
        with manifest_path.open(newline="") as f:
            for row in csv.DictReader(f):
                if (
                    ...
                    return path
    if os.environ.get("HAND_DRAG_ALLOW_DEFAULT_TRAJECTORY") == "1":
...
def metric_row(task, episode, step, reward, done):
    ...
    return {
        ...
        "action_clip099": int(bool(action_abs_max.item() >= 0.99)),
        "action_l2": float(action_l2.item()),
        "r_detach": float(task._r_detach_last[0].item()),
        "detach": int(bool(task._r_detach_last[0].item() < 0.0)),
    }
...
def summarize_episode(rows):
    ...
    return {
        ...
        "clip099": sum(row["action_clip099"] for row in rows) / len(rows),
        "detach": int(any(row["detach"] for row in rows)),
    }
...
def main():
    ...
    try:
        ...
        summary = {
            ...
            "clip099": mean(episodes, "clip099"),
            "detach_rate": mean(episodes, "detach"),
            "stochastic": bool(args.stochastic),
            "eval_protocol": "unified_main_table_v20260520",
        ...
        print(f"  clip099: {summary['clip099']:.3f}")
        print(f"  detach rate: {summary['detach_rate']:.3f}")
        if args.log_csv:
