...
class HandDragTask:
    ...
    def __init__(
        ...
        self.include_history_obs = bool(include_history_obs)
        ...
        history_obs_dim = (
            self.history_len * self.history_token_dim if self.include_history_obs else 0
        )
        self.obs_dim = base_obs_dim + history_obs_dim + self._aux_dim
        ...
        # ---- PICA v2c: causal-horizon ring buffers ----
        # Holds the last K+1 samples of palm_to_handle_dist and target object
        # joint position so we can compute past-window targets aligned with
        # the current obs. K is read from the config; default 5. Buffers are
        # always allocated (size K+1 even when aux is disabled) so the code
        # path is uniform; they are simply unread when not in causal_horizon
        # mode. The init value 0.0 is overwritten on the first reset_idx.
        ...
        self._palm_dist_hist = torch.zeros(
            self.num_envs, K + 1, device=self.device,
        )
    ...
    def reset_idx(self, env_ids: torch.Tensor):
        ...
        # palm distance and target-joint position. Filling all K+1 slots with
        # the reset value keeps q_response_K / max_dist_K / detach_proxy_K at
        # their natural reset values for the first K steps (response = 0,
        # max_dist = reset_dist, detach_proxy depends on reset state).
        post_reset_q_obj = self.dof_pos[
            env_ids, N_HAND_DOFS + self.target_joint_idx, 0,
        ]
        # broadcast scalar-per-env to all K+1 slots
    ...
    def compute_observations(self) -> torch.Tensor:
        ...
        if self.include_history_obs:
            obs_parts.append(flat_history)
        ...
        if self._phys_aux_enabled:
            ...
            # PICA v2c: roll ring buffers FIRST so the most recent slot is t.
            # We always update them when aux is enabled (cheap; needed for
            # any past-window target). For "current" mode they're harmless.
            self._palm_dist_hist = torch.roll(
                self._palm_dist_hist, shifts=-1, dims=1,
            )
            self._palm_dist_hist[:, -1] = self.palm_to_handle_dist
            self._q_obj_hist = torch.roll(self._q_obj_hist, shifts=-1, dims=1)
            self._q_obj_hist[:, -1] = cur_q_obj

            cols = []
            for k in keys:
                if k == "dq_obj":
                    # v2b: 1-step delta against prev_target_joint (compute_reward
                    # caches it BEFORE this is called -- prev_target_joint at
                    # this point holds q_obj_{t-1}).
                    col = (cur_q_obj - self.prev_target_joint).unsqueeze(-1)
                elif k == "slip_proxy":
                    # v2b: clamp(palm_dist - safe_dist, min=0)
                    col = (
                        self.palm_to_handle_dist - 0.08
                    ).clamp(min=0.0).unsqueeze(-1)
                elif k == "q_response_K":
                    # v2c: q_obj_t - q_obj_{t-K}
                    col = (
                        self._q_obj_hist[:, -1] - self._q_obj_hist[:, 0]
                    ).unsqueeze(-1)
                elif k == "max_dist_K":
                    # v2c: max palm_dist over the inclusive window [t-K..t]
                    col = self._palm_dist_hist.max(dim=1).values.unsqueeze(-1)
                elif k == "detach_proxy_K":
                    # v2c: 1.0 if max_dist over window > detach_dist else 0.0
                    max_d = self._palm_dist_hist.max(dim=1).values
                    col = (max_d > self.detach_dist).float().unsqueeze(-1)
                elif k == "tracking_stress":
                    # v2b/v2c shared: E_t = ||q_target - q|| over hand DOFs.
                    col = current_error.norm(dim=-1, keepdim=True)
                else:
                    # Unknown key (config mismatch); emit zeros to keep shapes
                    # consistent rather than crash mid-rollout.
                    col = torch.zeros(self.num_envs, 1, device=self.device)
                cols.append(col)
            aux_block = torch.cat(cols, dim=-1)
            self._aux_targets_last = aux_block.detach()
            # Compute (but do NOT apply) the contact gate for debug logging.
    ...
    def compute_reward(self):
        ...
        # ---- Total reward (PICA penalties added additively; zero by default) ----
        self.rew_buf = (
            r_dist + r_task + r_act + r_detach + r_success + r_time + r_phys_total
        )
