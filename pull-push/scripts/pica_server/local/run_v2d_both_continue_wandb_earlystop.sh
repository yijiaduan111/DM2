...
RUN_NAME=${RUN_NAME:-v2c150_v2d_both_continue400_45936_from_best}
GPU=${GPU:-0}
...
EARLY_MIN_EPOCHS=${EARLY_MIN_EPOCHS:-260}
EARLY_PATIENCE=${EARLY_PATIENCE:-50}
EARLY_MIN_DELTA=${EARLY_MIN_DELTA:-1.0}
EARLY_WINDOW=${EARLY_WINDOW:-5}
MONITOR_STEP_OFFSET=${MONITOR_STEP_OFFSET:-0}
WANDB_PROJECT=${WANDB_PROJECT:-pica-pull-push}
...
if [[ -n ${CHECKPOINT} && ! -e ${CHECKPOINT} ]]; then
  echo [launcher][error] checkpoint not found: ${CHECKPOINT} >&2
  exit 2
fi
...
MONITOR_STEP_OFFSET=${MONITOR_STEP_OFFSET}
WANDB_PROJECT=${WANDB_PROJECT}
WANDB_MODE=${WANDB_MODE}
EOF

TRAIN_ARGS=(ppo/train.py --train_config ${TRAIN_CFG} --object_id ${OBJECT_ID} --num_envs ${NUM_ENVS} --max_epochs ${MAX_EPOCHS} --bounds_loss_coef ${BOUNDS_LOSS_COEF} --experiment_name ${RUN_NAME})
if [[ -n ${CHECKPOINT} ]]; then
  TRAIN_ARGS+=(--checkpoint ${CHECKPOINT} --checkpoint-kind ${CHECKPOINT_KIND})
fi

echo [launcher] starting ${RUN_NAME} on GPU ${GPU} max_epochs=${MAX_EPOCHS} checkpoint=${CHECKPOINT}
CUDA_VISIBLE_DEVICES=${GPU} python -X faulthandler ${TRAIN_ARGS[@]} > logs_output/${RUN_NAME}_train.log 2>&1 &
TRAIN_PID=$!
echo ${TRAIN_PID} > logs_output/${RUN_NAME}_train.pid

echo "[launcher] train pid=${TRAIN_PID}; starting monitor"
python scripts/pica_server/local/monitor_train_wandb_earlystop.py \
  ...
  --window ${EARLY_WINDOW} \
--step-offset ${MONITOR_STEP_OFFSET} \
  > logs_output/${RUN_NAME}_monitor.log 2>&1 &
MONITOR_PID=$!
echo ${MONITOR_PID} > logs_output/${RUN_NAME}_monitor.pid
